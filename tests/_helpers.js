 ...
 * @example
 * await pollForCondition(dom.window, () => !!dom.window.localStorage.getItem('nfg-templates'));
 */
...
/**
 * Open the Templates pane in the UI and return the pane element. This helper
 * clicks the right-aligned Templates tab and waits until the corresponding
 * `.tab-pane` is present in the DOM.
 *
 * @param {JSDOM} dom - The JSDOM instance returned by `setup()`.
 * @returns {Promise<Element>} The `.tab-pane` element for the Templates pane.
 */
async function openTemplatesPane(dom) {
  const win = dom.window;
  const tplTab = win.document.querySelector('.tab.right') || Array.from(win.document.querySelectorAll('.tab')).find((t) => t.textContent && t.textContent.trim() === 'Templates');
...
/**
 * Create a new custom template via the Templates pane UI and save it.
...
async function createAndSaveTemplate(dom, labelText) {
  ...
  saveBtn.click();
  // wait until the left-pane selects include the new template option
  await pollForSelector(win, `.tab-pane[data-index="0"] select option[value="${newId}"]`);
  // ensure templates pane is current and return values
  pane = await openTemplatesPane(dom);
  return { newId, tplSelect: pane.querySelector('select'), pane };
}

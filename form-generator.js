var NFG=(()=>{var s=(e,n)=>()=>(n||e((n={exports:{}}).exports,n),n.exports);var i=s((u,r)=>{r.exports=`<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
          ...
          exportBtn.addEventListener(
            ...
            wrapHandler(async () => {
              try {
                ...
                safe(() => seed.push('window.formConfig = ' + safeJson(formConfig) + ';'));
                safe(() =>
            ...
            wrapHandler(() => {
                ...
                if (m) {
                  ...
                  if (formConfig[idx]) {
                    ...
                    safe(() => {
                      ...
                      if (pane) {
                        const f = q(pane, 'form.generated-form');
                        if (f) {
                  ...
                  safe(() => {
                    for (let j = 0; j < formConfig.length; j++) {
                      if (formConfig[j] && formConfig[j]._templateId === t.id) {
                        ...
                        let hasValues = false;
                        safe(() => {
                          const pane = q(contents, \`.tab-pane[data-index='\${j}']\`);
                          if (pane) {
                            const f = q(pane, 'form.generated-form');
                            if (f) {
                              Array.from(f.elements).forEach((e) => {
              ...
              safe(() => {
                ...
                if (m2) {
                  ...
                  const pane2 = q(contents, \`.tab-pane[data-index='\${idx2}']\`);
                  if (pane2) {
                    const formEl = q(pane2, 'form.generated-form');
                    if (formEl && formConfig[idx2] && Array.isArray(formConfig[idx2].fields)) {
                      formConfig[idx2].fields.forEach((f) => {
                        safe(() => {
                          if (
                            q(formEl, \`[name="\${CSS.escape(f.name)}"]\`) ||
                            (f.type === 'steps' &&
                              q(formEl, \`[name="\${CSS.escape(f.name + '_combined')}" ]\`))
                          )
                            return;
                          const wrapper = document.createElement('div');
                          wrapper.className = 'field';
...
`});var c=s((exports,module)=>{function loadHtml(){try{let e=i();if(typeof e=="string"&&e.indexOf("<script")!==-1)return e}catch(e){}try{let nodeRequire=eval('typeof require === "function" ? require : null');if(nodeRequire){let e=nodeRequire("fs"),n=nodeRequire("path");return e.readFileSync(n.join(__dirname,"form-generator.html"),"utf8")}}catch(e){}return""}var html=loadHtml();function mount(e,n){let a=n||{};if(typeof e=="string"&&(e=document.querySelector(e)),!e||typeof e.appendChild!="function")throw new Error("mount(): first argument must be a DOM element or a selector that resolves to one");if(!html)throw new Error("mount(): runtime HTML is unavailable \u2014 run `npm run build:html`");let o=html;if(Array.isArray(a.formConfig)){let d="<script>window.formConfig = "+JSON.stringify(a.formConfig).replace(/</g,"\\u003c")+";<\/script>";o=o.replace(/<head(\s[^>]*)?>/i,p=>p+d)}let l=e.ownerDocument||(typeof document<"u"?document:null);if(!l)throw new Error("mount(): no document available to create the iframe");let t=l.createElement("iframe");return t.setAttribute("title","Note Form Generator"),t.style.border="0",t.style.width="100%",t.style.height=a.height||"100%",t.srcdoc=o,e.innerHTML="",e.appendChild(t),t}module.exports={html,mount};module.exports.default=module.exports});return c();})();
